...
from HoloOceanUtils.factor_graph.gtsam_utils import fg_solver
...
# TODO: Improve io of simulator results to provide good comparison between ground truth, IMU, and GTSAM
    ...
    # TODO: add intermediate gtsam results class
        # implemented in evaluate_utils_mod.py, but not fully functional (issues with umeyama alignment, specifically the size doesn't match)
    # TODO: create plotting options (i.e. make multiplots and error plots but only show a quick review plot)
...
GT_STR = "gt_traj"
GTSAM_ODOM = "odom"
GTSAM_RANGE = "range_odom"
GTSAM_BEARING = "bearing_odom"
GTSAM_ALL = "all"
GTSAM_LEADING_STRS = [GTSAM_ODOM, GTSAM_RANGE, GTSAM_BEARING, GTSAM_ALL]
...
GTSAM_ODOM_TRAJ_LABEL = "IMU Odometry only"
GTSAM_RANGE_TRAJ_LABEL = "Range + Odometry"
GTSAM_BEARING_TRAJ_LABEL = "Bearing + Odometry"
GTSAM_ALL_TRAJ_LABEL = "Range + Bearing"

PREFERRED_LABEL_ORDERING = [
    GT_TRAJ_LABEL,
    GTSAM_ODOM_TRAJ_LABEL,
    GTSAM_RANGE_TRAJ_LABEL,
    GTSAM_BEARING_TRAJ_LABEL,
    GTSAM_ALL_TRAJ_LABEL,
]

TRAJ_LABEL_TO_COLOR = {
    GT_TRAJ_LABEL: "gray",
    GTSAM_ODOM_TRAJ_LABEL: "blue",
    GTSAM_RANGE_TRAJ_LABEL: "green",
    GTSAM_BEARING_TRAJ_LABEL: "goldenrod",
    GTSAM_ALL_TRAJ_LABEL: "darkorange",
}

@define
class ResultsPoseTrajCollection:
    """The results of a single trial
    Includes containers for ground truth, vehicle odometry, and 
    gtsam trajectories creating uses different sensor method combinations
    """
    gt_traj: PoseTrajectory3D = field()
    odom_traj: PoseTrajectory3D = field()
    gtsam_range: PoseTrajectory3D = field()
    gtsam_bearing: PoseTrajectory3D = field()
    gtsam_all: PoseTrajectory3D = field()
...
class TrajErrorDfs:
    ...
    # IMU Odometry only  0.911305  0.777825  0.661960  0.474831  0.050085  1.906614  1456.655153
    # Range + Odometry   0.911305  0.777825  0.661960  0.474831  0.050085  1.906614  1456.655153
    # Bearing + Odometry 0.911305  0.777825  0.661960  0.474831  0.050085  1.906614  1456.655153
    # Range + Bearing    0.911305  0.777825  0.661960  0.474831  0.050085  1.906614  1456.655153
    """
    ...
    if exists(aligned_results_pickle_path) and use_cached_results:
        ...
        for gtsam_leading_str in gtsam_experiments:
            if _should_write_new_tum_files(
                ...
                solver_result = fg_solver(pyfg,LM_SOLVER,gtsam_leading_str)
                tum_name = gtsam_leading_str + ".tum"
                modified_fpath = join(results_dir,tum_name)
                save_to_tum(solver_result, modified_fpath)
            else:
        ...
        aligned_results = ResultsPoseTrajCollection(
            gt_traj = gt_traj,
            odom_traj = aligned_joined_trajs[GTSAM_ODOM],
            gtsam_range = aligned_joined_trajs[GTSAM_RANGE],
            gtsam_bearing = aligned_joined_trajs[GTSAM_BEARING],
            gtsam_all = aligned_joined_trajs[GTSAM_ALL],
        )
...
def save_ape_error_stats_to_file(
        error_dfs: TrajErrorDfs,
        results_dir: str,
        use_cached_results: bool,
):
    fpath = join(results_dir, f"error_dfs.txt")
    indices = error_dfs.expected_indices
    cols = error_dfs.expected_cols
    trans_error = error_dfs.trans_error_df.round(1)
    rot_error = error_dfs.rot_error_df.round(1)
    pose_error = error_dfs.pose_error_df.round(1)
    error_df = pd.concat([pose_error, trans_error, rot_error])
    error_df.to_csv(fpath, sep='\t', index = True)
    with open(fpath, "r") as f:
        lines = f.readlines()
        del lines[0]
    lines.insert(0,"Pose Error (m) \trmse \tmean \tmedian std \tmin \tmax \tsse \n")
    lines.insert(5,"\n")
    lines.insert(6,"Translation Error (m) \trmse \tmean \tmedian std \tmin \tmax \tsse \n")
    lines.insert(11,"\n")
    lines.insert(12,"Rotation Error (deg) \trmse \tmean \tmedian std \tmin \tmax \tsse \n")
    with open(fpath, "w") as f:
        lines = "".join(lines)
        f.write(lines)

def scenario_plots(results_dir):
    aligned_results = get_aligned_traj_results_in_dir(results_dir,False)
    make_evo_traj_plots(aligned_results,results_dir,True,[plot.PlotMode.xy],False)
    make_evo_ape_plots_from_trajs(aligned_results, results_dir, False, True)
    error_df = get_ape_error_stats_from_aligned_trajs(aligned_results, results_dir, False)
    save_ape_error_stats_to_file(error_df, results_dir, False)