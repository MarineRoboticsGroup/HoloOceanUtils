...
# TODO: Improve io of simulator results to provide good comparison between ground truth, IMU, and GTSAM
    ...
    # TODO: Plot gt, odom, and gtsam-final
        # Issue with how the plot function handles multiple comparison methods
        # It can generate all of the .tum files, but only plots one set
        # TODO: Add options to plot different gtsam sensor methods
    # Worth adapting the plotting function to have results classes according to the different solver methods, 
...
GTSAM_LEADING_STRS = [ODOM_STR, GTSAM_FINAL]
...
class ResultsPoseTrajCollection:
    ...
    def comparison_trajs(self) -> List[PoseTrajectory3D]:
        ...
        if self.multi_robot_trajs_available:
            comparison_trajs.append(self.odom)
            comparison_trajs.append(self.gtsam_final)

        return comparison_trajs

    @property
    def comparison_traj_names(self) -> List[str]:
        names = [ODOM_TRAJ_LABEL,
            GTSAM_FINAL_TRAJ_LABEL,
        ]
        if self.multi_robot_trajs_available:
            names.append(ODOM_TRAJ_LABEL)
            names.append(GTSAM_FINAL_TRAJ_LABEL)
        else:
...
# TODO: Verify that this correctly sources the already made GTSAM .tum.
        ...
        for gtsam_leading_str in gtsam_experiments:
            if _should_write_new_tum_files(
                ...
                gtsam_tum_fpath = "/home/morrisjp/Documents/git/HoloOceanUtils/HoloOceanUtils/automated_experiments/example/experiment_1/gtsam_final.tum"
                save_to_tum(gtsam_result, gtsam_tum_fpath)

                odom_result = fg_solver(pyfg, LM_SOLVER, "odom")
                odom_tum_fpath = "/home/morrisjp/Documents/git/HoloOceanUtils/HoloOceanUtils/automated_experiments/example/experiment_1/odom.tum"
                save_to_tum(odom_result, odom_tum_fpath)
        ...
        aligned_results = ResultsPoseTrajCollection(
            gt_traj = gt_traj,
            odom = aligned_joined_trajs[ODOM_STR],
