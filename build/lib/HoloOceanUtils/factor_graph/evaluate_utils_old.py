...
# TODO: Improve io of simulator results to provide good comparison between ground truth, IMU, and GTSAM
    ...
    # TODO: add intermediate gtsam results class
        # implemented, but not functional(gives 59 instead of 60 poses, so it can't be aligned with the gtsam_final results)
    # TODO: add naive odometry results (and plot)
    # TODO: Consider implementing a downsampling option in the align results function
        # intermediate is very noisy at 1Hz, but is fine at .1Hz
        # evo has a basic downsampler, but some sort of smoothing would be best
        # i.e. don't use every 10th datapoint, use the average of those 10
        # easiest way to do that would be to slow down the intermediate analysis freq (in trial_runner), then downsample the other two traj to match
    # TODO: Revise plot function to allow for plotting the agents as two different colors
    # TODO: Save stats as a text file
    # TODO: Consider adding data class for method comparison (range, bearing, both)
...
# Trajectory Types
GT_STR = "gt_traj"
GTSAM_FINAL = "gtsam_final"
GTSAM_INT = "gtsam_int"
GTSAM_LEADING_STRS = [GTSAM_FINAL, GTSAM_INT]
...
GTSAM_FINAL_TRAJ_LABEL = "GTSAM Final"
GTSAM_INT_TRAJ_LABEL = "GTSAM Intermediate"

PREFERRED_LABEL_ORDERING = [
    GT_TRAJ_LABEL,
    GTSAM_FINAL_TRAJ_LABEL,
    GTSAM_INT_TRAJ_LABEL,
]

TRAJ_LABEL_TO_COLOR = {
    GT_TRAJ_LABEL: "gray",
    GTSAM_FINAL_TRAJ_LABEL: "blue",
    GTSAM_INT_TRAJ_LABEL: "green",
}
...
class ResultsPoseTrajCollection:
    ...
    gtsam_final: PoseTrajectory3D = field()
    gtsam_int: PoseTrajectory3D = field()
    ...
    def comparison_traj_names(self) -> List[str]:
        names = [
            GTSAM_FINAL_TRAJ_LABEL, GTSAM_INT_TRAJ_LABEL
        ]
        if self.multi_robot_trajs_available:
            names.append(GTSAM_FINAL_TRAJ_LABEL, GTSAM_INT_TRAJ_LABEL)
        else:
    ...
    # TODO: Verify that this correctly sources the already made GTSAM .tum.
        ...
        for gtsam_leading_str in gtsam_experiments:
            if _should_write_new_tum_files(
                ...
                gtsam_result = fg_solver(pyfg, LM_SOLVER, "all")
                tum_fpath = "/home/morrisjp/Documents/git/HoloOceanUtils/HoloOceanUtils/automated_experiments/example/experiment_1/gtsam_final.tum"
        ...
        # If gtsam-int and gtsam-final are off by one line, copy the gtsam-final point over
        def correct_gtsam_traj_length(gtsam_final: str, gtsam_int: str):
            with open(gtsam_final, "r") as f:
                for line in f:
                    pass
                last_line = line
                f.close()
            with open(gtsam_int, "a+") as f:
                f.write(last_line)
                f.close()
            return gtsam_int
        correct_gtsam_traj_length(
            "/home/morrisjp/Documents/git/HoloOceanUtils/HoloOceanUtils/automated_experiments/example/experiment_1/gtsam_final_A.tum",
            "/home/morrisjp/Documents/git/HoloOceanUtils/HoloOceanUtils/automated_experiments/example/experiment_1/gtsam_int_A.tum")
        correct_gtsam_traj_length(
            "/home/morrisjp/Documents/git/HoloOceanUtils/HoloOceanUtils/automated_experiments/example/experiment_1/gtsam_final_B.tum",
            "/home/morrisjp/Documents/git/HoloOceanUtils/HoloOceanUtils/automated_experiments/example/experiment_1/gtsam_int_B.tum")
        ...
        aligned_results = ResultsPoseTrajCollection(
            gt_traj = gt_traj,
            gtsam_final = aligned_joined_trajs[GTSAM_FINAL],
            gtsam_int = aligned_joined_trajs[GTSAM_INT],
        )
